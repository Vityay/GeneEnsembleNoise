...
load('CAP_sepsis_Ensemble.Rdat')
XGB_model_bst <- xgb.load('CAP_sepsis_Ensemble.model')
...
pdf("Figure3D.pdf", width=6, height=3.5)
...
dev.off()
################################################################################
ggsave(filename="Table2_CAP_sepsis_Ensemble_XGB.pdf", grid.arrange(tableGrob(conf_table)),
        width = 6, height = 4, units = "cm")
################################################################################

pheno_train <- pheno[inTraining,]
pheno_valid <- pheno[-inTraining,]
...
pdf("Figure3C.pdf", paper="a4", width=3.5, height=3.7)
print(p1, newpage=F)
print(p2, newpage=T)
dev.off()
