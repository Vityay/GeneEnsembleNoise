...
load('Sepsis_WGCNA_XGB.Rdat')
XGB_model_bst <- xgb.load('Sepsis_WGCNA_XGB.model')
...
pdf("FigureS9A.pdf", width=4, height=9)

# k <- which(colors == "grey")+1
# m <- length(colors)
par(mar=c(4,16,1,0.5))
barplot(var_imp[,1], horiz = T, names.arg = rownames(var_imp), col = col2hex(colors),
...
################################################################################
pdf("FigureS6C.pdf", width=6, height=6)
...
setwd("/media/yuri/Data/home1/GNI_data/infection/ResultsSciRep/Figures/")
pdf("FigureS9B.pdf", width=4, height=9)
  # k <- max(which(topME_ens[,'module'] == "grey"))+1
  # m <- length(topME_ens[,'module'])
  par(mar=c(4,16,1,0.5))
  barplot(topME_ens[,'fdr'], horiz = T, names.arg = topME_ens[,'ensemble'], col = col2hex(topME_ens[,'module']),
