...
load('Sepsis_Ensemble.Rdat')
XGB_model_bst <- xgb.load('Sepsis_Ensemble.model')
...
pdf("Figure5D.pdf", width=6, height=3.5)
...
dev.off()
################################################################################
pdf("Figure5B.pdf", width=6, height=6)
...
dev.off()
################################################################################
ggsave(filename="Table2_SPS_Ensemble_XGB.pdf", grid.arrange(tableGrob(conf_table)),
        width = 6, height = 4, units = "cm")
################################################################################

pheno_train <- pheno_SPS[inTraining,]
pheno_valid <- pheno_SPS[-inTraining,]
...
pdf("Figure5C.pdf", paper="a4", width=3.5, height=3.7)
print(p1, newpage=F)
print(p2, newpage=T)
dev.off()
