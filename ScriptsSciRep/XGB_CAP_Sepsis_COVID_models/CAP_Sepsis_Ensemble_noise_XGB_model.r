...
rm(affyDat_noise_paths_CAP, affyDat_noise_paths_SPS, pheno_CAP, pheno_SPS)
################################################################################
# Split data into discovery and validation as in GSE65682
inTraining <- which(pheno$cohort == "discovery")

# select significant features by t.test from the discovery cohort
features <- sapply(seq(ncol(affyDat_noise_paths)), function(i)
    t.test(affyDat_noise_paths[inTraining,i] ~ pheno$mort[inTraining])$p.value )


X <- data.frame(affyDat_noise_paths[, features <= 0.01], age=pheno$age )
# save feature names
ids <- colnames(X)
...
save.image('CAP_sepsis_Ensemble.Rdat')
xgb.save(XGB_model_bst, 'CAP_sepsis_Ensemble.model')
