...
rm(affyDat_noise_paths, affyDat_noise_paths_SPS, pheno, pheno_SPS)
################################################################################

inTraining <- which(pheno_CAP$cohort == "discovery")

features <- sapply(seq(ncol(affyDat_noise_paths_CAP)), function(i)
    t.test(affyDat_noise_paths_CAP[inTraining,i] ~ pheno_CAP$mort[inTraining])$p.value )


X <- data.frame(affyDat_noise_paths_CAP[, features <= 0.05], pheno=pheno_CAP$age )
ids <- colnames(X)
colnames(X) <- paste0("x", seq(ncol(X)))
Y <- factor(pheno_CAP$mort)
...
save.image('CAP_Ensemble.Rdat')
xgb.save(XGB_model_bst, 'CAP_Ensemble.model')
